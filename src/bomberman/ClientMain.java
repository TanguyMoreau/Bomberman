        ...
        public static void main(String[] args) throws RemoteException, NotBoundException{
                ...
                while(true){
                        ...
                        if(c.getClient().getMyInterface().getInfoFromClientsPos(c.getClient().getPosition()).isDataSend() == false){
                                Action newAction = c.getWindow().getListener().getAction();
                                c.getWindow().getListener().setAction(new Action(new Coordinates(), false));
//new Action(new Coordinates(3 * (1 - 2 * r.nextDouble()), 3 * (1 - 2  *r.nextDouble())), false);
                                try{
